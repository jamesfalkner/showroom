...
class WorkshopChatbot {
  constructor() {
    ...
    this.config = {
      ...
      mcpNamespace: 'default' // Default Kubernetes namespace
    };
      ...
      '<div id="chatbot-window" class="chatbot-window">' +
        ...
        '<div id="chatbot-settings-panel" class="chatbot-settings-panel">' +
          ...
          '<div id="mcp-settings-group" style="display: block;">' +
            '<div class="setting-group">' +
              '<label for="mcp-namespace">Default Namespace:</label>' +
              '<input type="text" id="mcp-namespace" value="default" placeholder="default">' +
              '<small>Kubernetes namespace for operations</small>' +
            '</div>' +
            '<small style="color: #666; font-style: italic;">Note: MCP server connection is handled by the backend</small>' +
          '</div>' +
        ...
        '<div class="chatbot-messages" id="chatbot-messages">' +
          '<div class="message bot-message">' +
            '<div class="message-content">' +
              '👋 Hi! I\'m your Platform Engineering Workshop assistant. I know all about the workshop content and can help you with:' +
              '<ul>' +
                '<li>Questions about Red Hat Developer Hub</li>' +
                '<li>Platform Engineering concepts</li>' +
                '<li>Workshop activities and steps</li>' +
                '<li>Troubleshooting guidance</li>' +
              '</ul>' +
              '<em>Configure your AI endpoint in settings (⚙️) to get started!</em>' +
            '</div>' +
          '</div>' +
        '</div>' +
  ...
  saveSettings() {
    ...
    const mcpNamespaceEl = document.getElementById('mcp-namespace');
    
    if (!mcpEnabledEl || !mcpNamespaceEl) {
    ...
    const mcpNamespace = mcpNamespaceEl.value;

    this.config = {
      ...this.config,
      mcpEnabled: mcpEnabled,
      mcpNamespace: mcpNamespace || 'default'
    };
  ...
  loadSettings() {
    ...
    if (saved) {
      ...
      document.getElementById('mcp-namespace').value = this.config.mcpNamespace || 'default';
  ...
  getCurrentPageContext() {
    ...
    if (pageContext) {
      pageContext = pageContext
        ...
        .replace(/\s*- Platform Engineering Workshop$/, '')
        .trim();
    }
  ...
  // Legacy method - no longer used since frontend uses streaming via backend
  async callAI(userMessage) {
    throw new Error('Direct AI calls from frontend are deprecated. Use streamChatResponse instead.');
  }

  buildSystemPrompt(userMessage = '') {
    // System prompt is now handled entirely by the backend
    // This method is kept for compatibility but is no longer used
    // The backend builds the system prompt from external configuration
    return '';
  }

  // Helper method to find relevant workshop content based on user query
  findRelevantContent(query, maxTokens = 6000) {
    if (!this.workshopContent) {
      return 'Workshop content is loading...';
    }
    ...
    const topicKeywords = {
      'authentication': ['auth', 'login', 'oauth', 'keycloak', 'sso', 'signin', 'oidc'],
      'deployment': ['deploy', 'pod', 'container', 'kubernetes', 'openshift', 'helm'],
      'development': ['code', 'develop', 'build', 'ci', 'pipeline', 'git', 'template'],
      'configuration': ['config', 'setup', 'install', 'configure', 'settings'],
      'monitoring': ['monitor', 'observ', 'metrics', 'logs', 'alert'],
      'networking': ['network', 'ingress', 'route', 'service', 'endpoint'],
      'storage': ['storage', 'volume', 'persistent', 'database', 'data'],
      'security': ['security', 'rbac', 'permission', 'access', 'policy'],
      'backstage': ['backstage', 'rhdh', 'developer hub', 'catalog', 'component'],
      'gitops': ['gitops', 'argo', 'argocd', 'sync', 'application'],
      'scaffolding': ['scaffold', 'template', 'create', 'generate', 'software template']
    };
    
    // Find which topics are relevant to the query
    ...
    // Convert workshop content to string if it's an object
    const contentStr = typeof this.workshopContent === 'string' ? 
      this.workshopContent : 
      JSON.stringify(this.workshopContent, null, 2);
    
    // Split content into sections (by modules or major headings)
    const sections = contentStr.split(/(?=##?\s+Module|##?\s+Chapter|##?\s+Section)/i);
    ...
    const scoredSections = sections.map(section => {
      ...
      // Direct keyword matches from query
      ...
      // Boost current page content
      const currentPage = document.querySelector('h1')?.textContent || '';
      if (currentPage && sectionLower.includes(currentPage.toLowerCase())) {
        score += 5;
      }
    ...
    // Add sections until we hit the token limit (rough estimate: 4 chars = 1 token)
    for (const item of scoredSections) {
      const sectionTokens = Math.ceil(item.section.length / 4);
      if (tokenCount + sectionTokens > maxTokens) {
        break;
      }
      relevantSections.push(item.section);
      tokenCount += sectionTokens;
    }
    
    // If no relevant sections found or query is empty, include a summary
    if (relevantSections.length === 0 || !query.trim()) {
      const summary = contentStr.substring(0, Math.min(3000, contentStr.length));
      return 'Workshop Summary (first section):\n' + summary + '\n\n(Note: This is a partial view. Ask specific questions for more targeted help.)';
    }
  ...
  addMessage(content, sender, type = 'normal') {
    ...
    // Add to conversation history (only for user messages, since assistant responses are handled in streaming)
    if (type !== 'error' && sender === 'user') {
      this.chatHistory.push({
        role: 'user',
        content: content
      });
  ...
  async callMCPTool(toolName, parameters = {}) {
    if (!this.config.mcpEnabled || !this.config.mcpEndpoint) {
      throw new Error('MCP server not configured. Please enable and configure MCP in settings.');
    }

    // Handle different URL formats
    let mcpUrl = this.config.mcpEndpoint;
    if (mcpUrl.startsWith('/')) {
      // Relative URL - use current domain (includes /mcp-proxy)
      mcpUrl = window.location.origin + mcpUrl;
    } else if (mcpUrl.startsWith('http://mcp-server:8080')) {
      // Internal service URL - convert to external route
      mcpUrl = mcpUrl.replace('http://mcp-server:8080', 'https://mcp-server-workshop-mcp.apps.cluster-n2sn6.n2sn6.sandbox1224.opentlc.com');
    } else if (!mcpUrl.includes('/mcp') && !mcpUrl.includes('mcp-proxy')) {
      // Add /mcp path if not present and not using proxy
      mcpUrl = mcpUrl.endsWith('/') ? mcpUrl + 'mcp' : mcpUrl + '/mcp';
    }

    console.log('Calling MCP tool:', { toolName, parameters, endpoint: mcpUrl });
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error('MCP call failed: ' + response.status + ' ' + response.statusText + '. Details: ' + errorText);
      }
  ...
  async getMCPTools() {
    ...
    try {
      ...
      if (!response.ok) {
        console.warn('Failed to get MCP tools:', response.status);
        return [];
      }
  ...
  // Legacy method - Enhanced AI calls with MCP are deprecated
  async enhancedAICall(userMessage) {
    throw new Error('Enhanced AI calls from frontend are deprecated. MCP integration is now handled by the backend.');
  }
